...
CHUNK_SIZE = 200_000
MAX_OBSERVED_YEARS = 31.509924709103355  # from core training data, same value used for the core population
...
def score_chunk(chunk, lookups, rsid_lookup, af_lookup, reclass_model, reclass_feature_cols,
                 direction_models, direction_feature_cols, cox_model, survival_feature_cols, aft_model):
    features = chunk.copy()
    ...
    features = features.merge(af_lookup, on="VariationID", how="left")
    features["gnomad_af"] = features["gnomad_af_real"].fillna(0.0)
    features["gnomad_af_popmax"] = features["gnomad_af_popmax_real"].fillna(0.0)
    features["gnomad_af_log"] = np.log1p(features["gnomad_af"])
    features["gnomad_af_popmax_log"] = np.log1p(features["gnomad_af_popmax"])
    features["annovar_exonic_func"] = features["annovar_exonic_func_real"].fillna("unknown")
    features = features.drop(columns=["gnomad_af_real", "gnomad_af_popmax_real", "annovar_exonic_func_real", "annovar_func_region_real"])

    for col in EXONIC_FUNC_DUMMIES:
        features[col] = 0
    for raw_value, dummy_col in EXONIC_FUNC_TO_DUMMY.items():
        features.loc[features["annovar_exonic_func"] == raw_value, dummy_col] = 1
    ...
    features["reclass_probability"] = reclass_model.predict_proba(features[reclass_feature_cols])[:, 1]
    ...
    # the actual backfill: AFT median + extrapolation flag
    features["median_years_to_reclass_aft"] = aft_model.predict_median(features[survival_feature_cols]).values
    features["extrapolated_beyond_observed_range"] = features["median_years_to_reclass_aft"] > MAX_OBSERVED_YEARS

    features["cohort"] = "extended_new_since_2019"
    features["feature_completeness"] = "real_af_exonic_func_via_annovar_gnomad211_exome"
    features["scoring_model"] = "v15_realaf"
...
if __name__ == "__main__":
    ...
    print(f"extended population (NEW since 2019): {len(new_vus)}")
    del current_vus
    gc.collect()
    ...
    output_path = "data/stage2/vus_watchlist_extended_new_since_2019_v15_realaf.csv.gz"
    n_chunks = (len(new_vus) + CHUNK_SIZE - 1) // CHUNK_SIZE
    first_chunk = True
    for i in range(n_chunks):
        chunk = new_vus.iloc[i * CHUNK_SIZE:(i + 1) * CHUNK_SIZE]
        scored = score_chunk(chunk, lookups, rsid_lookup, af_lookup, reclass_model, reclass_feature_cols,
                              direction_models, direction_feature_cols, cox_model, survival_feature_cols, aft_model)
        scored = scored.rename(columns={"VariationID": "ClinvarID"})
    ...
    # sanity check
    check = pd.read_csv(output_path, low_memory=False, usecols=["median_years_to_reclass_aft", "extrapolated_beyond_observed_range"])
    print(f"\nnull median_years_to_reclass_aft: {check['median_years_to_reclass_aft'].isna().sum()} / {len(check)}")
    print(f"median_years_to_reclass_aft: min={check['median_years_to_reclass_aft'].min():.2f}, "
          f"median={check['median_years_to_reclass_aft'].median():.2f}, max={check['median_years_to_reclass_aft'].max():.2f}")
    print(f"extrapolated_beyond_observed_range: {check['extrapolated_beyond_observed_range'].sum()} rows "
          f"({check['extrapolated_beyond_observed_range'].mean()*100:.1f}%)")
